...
class BaseEncoder(BaseTriton):
    ...
    def forwark(self, input_data, model_name='encoder'):
...
class BaseEmbed(BaseTriton):
    ...
    def forwark(self, input_data, model_name='embed'):
...
class BaseLinear(BaseTriton):
    ...
    def forwark(self, input_data, model_name='linear'):
...
class BaseLogsoftmax(BaseTriton):
    ...
    def forwark(self, input_data, model_name='logsoftmax'):
...
class BaseYolov7(BaseTriton):
    def __init__(self, input_name=['images'], input_type=['FP32'], input_dim=[[3, 640, 640]], output_name=['det_boxes', 'det_classes', 'det_scores', 'num_dets'], url=URL, verbose=False, ssl=False, root_certificates=None, private_key=None, certificate_chain=None, client_timeout=None, grpc_compression_algorithm=None, static=False):
        super().__init__(input_name, input_type, input_dim, output_name, url, verbose, ssl, root_certificates, private_key, certificate_chain, client_timeout, grpc_compression_algorithm, static)

    def forwark(self, input_data, model_name='yolov7'):
        output = self.predict(inputs_data=input_data,model_name='yolov7')
        return output